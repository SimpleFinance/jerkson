...
class CaseClassDeserializer(config: DeserializationConfig,
  ...
  private val isSnakeCase = javaType.getRawClass.isAnnotationPresent(classOf[JsonSnakeCase]) || javaType.getRawClass.isAnnotationPresent(classOf[CodaSnakeCase])
  ...
  private def errorMessage(node: JsonNode) = {
    ...
    val needed: Set[String] = params.map { _._1 }.toSet
    val found: Set[String] = node match {
      case obj: ObjectNode => obj.fieldNames.map(_.toString).toSet
      case _ => Set()
    }

    "Invalid JSON. Missing fields %s; needed %s, found %s.".format(
      needed.diff(found).mkString("[", ", ", "]"), names, existing)
  }
